...
def get_div_data(symbol, custom_div_info=None):
    ...
    try:
        ...
        if custom_div_info and custom_div_info.get('v', 0) > 0:
            ...
            status_msg = "✅ 已公告 (手動)" if ex_date_obj.date() >= today.date() else "✅ 前次紀錄 (手動)"
            ...
            if official_amount > 0: div_amount = official_amount
                ...
                if not actions.empty:
                    div_amount = float(actions.sort_index(ascending=False).head(1)['Dividends'].values[0])
            ex_date_obj = datetime.strptime(ex_date, '%Y-%m-%d')
            status_msg = "✅ 已公告 (台灣官方)" if ex_date_obj.date() >= today.date() else "✅ 前次配息 (台灣官方)"
...
def fetch_data(etf_list, custom_divs):
    ...
    for item in etf_list:
        try:
            ...
            tech_results.append({
                "ETF 名稱": display_name, "配息月份": month_tag, "股票張數": item['holdings'], "現價": round(curr_p, 2),
                "今日損益": today_pnl_str, "今日漲跌幅": today_pct_str, "今日交易量": f"{vol:,.0f}" if vol > 0 else "無資料",
                "年殖利率": f"{est_yield:.2f}%", "今日最高/最低": f"${day_high:.2f} / ${day_low:.2f}",
...
if price_alerts:
    for alert in price_alerts:
        st.markdown(f"<div class='alert-{'high' if alert['type']=='high' else 'low'}'>{'🚨 突破停利高標' if alert['type']=='high' else '⚠️ 跌破停損低標'}：【{alert['name']}】 現價 ${alert['price']:.2f} 已{'突破' if alert['type']=='high' else '跌破'}您設定的 ${alert['target']}！</div>", unsafe_allow_html=True)
...
with col1:
    if radar_ex:
        ...
        st.markdown(f"<div class='ex-div-box' style='background-color: #f4f6f8; border: 1.5px dashed #adb5bd;'><div class='ex-div-title' style='color: #6c757d;'>📡 預測雷達 (等待官方公告)</div><div class='ex-div-text' style='color: #495057;'>{msg}</div></div>", unsafe_allow_html=True)
...
with col2:
    if radar_pay:
        ...
        st.markdown("<div class='pay-div-box' style='background-color: #fafafa; border: 1.5px dashed #ddd;'><div class='pay-div-title' style='color: #888;'>💰 領息雷達提醒 💰</div><div class='pay-div-text' style='color:#666;'>目前無 20 天內領息雷達提示</div></div>", unsafe_allow_html=True)
...
st.markdown(f'''<div class="triple-box"><div class="triple-col"><div class="triple-title">今日損益</div><div class="{today_c_val}">{today_val_str}</div><div class="{today_c_pct}">{today_pct_str}</div></div><div class="triple-col"><div class="triple-title">累積預估淨損益 (已扣手續費/稅)</div><div class="{total_c_val}">{total_val_str}</div><div class="{total_c_pct}">{total_pct_str}</div></div><div class="triple-col flash-gold-box"><div class="triple-title" style="color: #b48608; margin-bottom: 5px;">⚡ {current_month_num} 月預估領息總額</div><div class="triple-val-gold">${current_month_div_amount:,.0f}</div><div class="triple-sub-gold">{sub_title}</div></div></div>''', unsafe_allow_html=True)
...
if "us" in macro_data and macro_data["us"]:
    ...
    us_icon = "🔴" if us_up >= (len(macro_data["us"]) - us_up) else "🟢"

tw_icon = "🇹🇼"
if "tw" in macro_data and macro_data["tw"]:
    tw_up = sum(1 for v in macro_data["tw"].values() if v['diff'] >= 0)
    tw_icon = "🔴" if tw_up >= (len(macro_data["tw"]) - tw_up) else "🟢"

cols_btn_r1 = st.columns(3); cols_btn_r2 = st.columns(3); cols_btn_r3 = st.columns(3); cols_btn_r4 = st.columns(3)

with cols_btn_r1[0]: st.button(f"🔽 收起美股指數 {us_icon}" if st.session_state.show_us else f"{us_icon} 展開美股指數", on_click=toggle_us, type="primary" if st.session_state.show_us else "secondary", use_container_width=True)
with cols_btn_r1[1]: st.button(f"🔽 收起台股指數 {tw_icon}" if st.session_state.show_tw else f"{tw_icon} 展開台股指數", on_click=toggle_tw, type="primary" if st.session_state.show_tw else "secondary", use_container_width=True)
with cols_btn_r1[2]: st.button("🔽 收起每月領息" if st.session_state.show_calendar else "📅 展開每月領息", on_click=toggle_calendar, type="primary" if st.session_state.show_calendar else "secondary", use_container_width=True)

with cols_btn_r2[0]: st.button("🔽 收起除權息" if st.session_state.show_div_db else "📂 展開除權息", on_click=toggle_div_db, type="primary" if st.session_state.show_div_db else "secondary", use_container_width=True)
with cols_btn_r2[1]: st.button("🔽 收起股價監控" if st.session_state.show_tech else "📡 展開股價監控", on_click=toggle_tech, type="primary" if st.session_state.show_tech else "secondary", use_container_width=True)
with cols_btn_r2[2]: st.button("🔽 收起持股明細" if st.session_state.show_holdings else "📊 展開持股明細", on_click=toggle_holdings, type="primary" if st.session_state.show_holdings else "secondary", use_container_width=True)

with cols_btn_r3[0]: st.button("🔽 收起ETF成份股" if st.session_state.show_constituents else "🧩 展開ETF成份股", on_click=toggle_constituents, type="primary" if st.session_state.show_constituents else "secondary", use_container_width=True) 
with cols_btn_r3[1]: st.button("🔽 收起質押專區" if st.session_state.show_pledge else "🏦 展開質押專區", on_click=toggle_pledge, type="primary" if st.session_state.show_pledge else "secondary", use_container_width=True) 
with cols_btn_r3[2]: st.button("🔽 收起機密面板" if st.session_state.show_secret else "🔐 展開機密面板", on_click=toggle_secret, type="primary" if st.session_state.show_secret else "secondary", use_container_width=True) 

with cols_btn_r4[0]: st.button("🔽 收起每日單日報表" if st.session_state.show_daily_price else "🗓️ 展開每日單日報表", on_click=toggle_daily_price, type="primary" if st.session_state.show_daily_price else "secondary", use_container_width=True) 
...
if st.session_state.show_calendar:
    ...
    selected_month = int(st.selectbox("請選擇您想查詢的月份：", month_options, index=datetime.today().month - 1).replace(" 月", ""))
    data = monthly_calendar[selected_month]
    sources_text = "、".join(data["sources"]) if data["sources"] else "本月無除息預定"
    col_space1, col_center, col_space2 = st.columns([1, 2, 1])
    with col_center:
        st.markdown(f"<div class='month-card'><div class='month-title'>{selected_month} 月預估領息</div><div class='month-amount'>${data['amount']:,.0f}</div><div class='month-sources'>ETF 來源：{sources_text}</div></div>", unsafe_allow_html=True)
    st.write("---")
...
if st.session_state.show_div_db:
    ...
    if not df.empty:
        for _, row in df.iterrows():
            ...
            freq = "月配息" if len(months)==12 else "季配息" if len(months)==4 else "半年配" if len(months)==2 else "年配息" if len(months)==1 else "未知"
            db_list.append({"類別": "💼 庫存", "ETF 名稱": row['名稱'], "基金規模": row.get('基金規模', '系統無資料'), "配息頻率": freq, "配息月份": "、".join(map(str, months)) + " 月" if months else "未設定", "狀態": row['狀態'], "除息日": row['最新公告除息日'], "發放日": row['預估發放日'], "每股金額": f"${row['每股配息']:.3f}", "最新填息紀錄": row['最新填息紀錄']})
    ...
    with st.expander("🛠️ 總司令專屬：手動配息覆蓋面板", expanded=False):
        ...
        edited_custom = st.data_editor(df_custom, num_rows="dynamic", use_container_width=True)
        if st.button("💾 儲存手動覆蓋資料並套用", type="primary"):
            ...
            st.session_state.my_data['custom_divs'] = new_db; save_to_json(st.session_state.my_data); st.cache_data.clear(); st.session_state.update_success = "套用成功！"; st.rerun()
    st.write("---")
...
if st.session_state.show_tech:
    if not df.empty:
        ...
        with auto_tech_col:
            st.markdown("<div style='background-color: #f0f7ff; border: 1px solid #cce5ff; border-radius: 8px; padding: 10px 8px; text-align: center;'><div style='font-size: 15px; font-weight: bold; color: #1e3c72; margin-bottom: 4px;'>⚡ 股價更新</div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<div style='font-size: 11px; color: #6c757d; margin-bottom: 5px;'>每 {refresh_sec} 秒重整</div>", unsafe_allow_html=True)
            if 'auto_refresh_mode' not in st.session_state: st.session_state.auto_refresh_mode = "❌ 關閉"
            st.radio("即時更新", ["❌ 關閉", "✅ 開啟"], key="auto_refresh_mode", horizontal=False, label_visibility="collapsed")
...
if st.session_state.show_constituents:
    if not df.empty:
        ...
        for idx, item in enumerate(st.session_state.my_data['etfs']):
            comp_data = ETF_CONSTITUENTS_DB.get(item['symbol'], [{"name": "其他成分股", "weight": 100.0}])
            df_comp = pd.DataFrame(comp_data)
            df_comp['label'] = df_comp['weight'].apply(lambda w: f"{w:.1f}%" if w >= 2.0 else "")
            base = alt.Chart(df_comp).encode(theta=alt.Theta("weight:Q", stack=True), color=alt.Color("name:N", sort=alt.EncodingSortField(field="weight", op="sum", order="descending"), legend=alt.Legend(title=None, orient="right", labelFontSize=12)), tooltip=["name", "weight"])
...
# ==============================================================================
# 🔥 [重磅升級] 🗓️ 5月份起 每日單日損益金額 & 每日收盤價 (加入每日合計與缺漏資料補正)
# ==============================================================================
...
if st.session_state.show_pledge:
    if not df.empty:
        ...
        pledge_df_list = []; total_p_mkt = 0; total_borrowable = 0
        for item in st.session_state.my_data['etfs']:
            sym = item['symbol']
            try: curr_p = df[df['代號'] == sym]['現價'].values[0]
            except: curr_p = 0
            p_shares = item.get('pledged_shares', 0.0); is_p = item.get('is_pledged', False)
            original_mkt = item['holdings'] * 1000 * curr_p
            p_mkt = p_shares * 1000 * curr_p if is_p else 0
            p_limit = p_mkt * 0.6 if is_p else 0
            total_p_mkt += p_mkt; total_borrowable += p_limit
            pledge_df_list.append({"✓ 選取": is_p, "ETF 名稱": item['name'], "總庫存 (張)": item['holdings'], "庫存市值 (元)": round(original_mkt, 0), "質押張數": p_shares, "現價": round(curr_p, 2), "質押市值 (元)": round(p_mkt, 0), "可借上限 (60%)": round(p_limit, 0)})
            
        m_ratio = (total_p_mkt / borrowed * 100) if borrowed > 0 else 0
        col_m1, col_m2, col_m3, col_m4 = st.columns(4)
        col_m1.metric("擔保品總市值", f"${total_p_mkt:,.0f}"); col_m2.metric("🎯 總可借款上限 (60%)", f"${total_borrowable:,.0f}"); col_m3.metric("💸 已借入總額", f"${borrowed:,.0f}")
        if borrowed > 0:
            if m_ratio < 130: col_m4.metric("🚨 目前維持率", f"{m_ratio:.2f}%", "危險：低於 130% 將面臨斷頭", delta_color="inverse")
            elif m_ratio < 160: col_m4.metric("⚠️ 目前維持率", f"{m_ratio:.2f}%", "注意：市場波動可能導致風險", delta_color="off")
            else: col_m4.metric("✅ 目前維持率", f"{m_ratio:.2f}%", "安全：維持率處於健康水平", delta_color="normal")
        else: col_m4.metric("目前維持率", "0.00%")

        edited_p = st.data_editor(pd.DataFrame(pledge_df_list), column_config={"✓ 選取": st.column_config.CheckboxColumn("✓ 選取質押"), "質押張數": st.column_config.NumberColumn("質押張數 (雙擊編輯)", min_value=0.0, step=1.0, format="%.1f")}, disabled=["ETF 名稱", "總庫存 (張)", "庫存市值 (元)", "現價", "質押市值 (元)", "可借上限 (60%)"], use_container_width=True, hide_index=True)
        has_p_changes = False
        for _, row in edited_p.iterrows():
            p_name = row['ETF 名稱']; new_is_pledged = row['✓ 選取']; new_p_shares = row['質押張數']
            for etf in st.session_state.my_data['etfs']:
                if etf['name'] == p_name:
                    if new_p_shares > etf['holdings']: new_p_shares = etf['holdings']
                    if etf.get('pledged_shares', 0.0) != new_p_shares or etf.get('is_pledged', False) != new_is_pledged:
                        etf['pledged_shares'] = new_p_shares; etf['is_pledged'] = new_is_pledged; has_p_changes = True
                    break
        if has_p_changes: save_to_json(st.session_state.my_data); st.rerun()
    else: st.info("⚠️ 目前無持股資料。")
    st.write("---")
...
if st.session_state.show_secret:
    ...
    if not st.session_state.is_unlocked:
        ...
        st.success("✅ 機密面板已解鎖！")
        st.markdown("##### 💳 核心信貸還款戰情 (每月 $15,000 / 7年共84期)")
        loan_info = st.session_state.my_data['loan']
        new_paid = st.slider("手動微調已繳納期數", min_value=1, max_value=84, value=int(loan_info['months_paid']))
        ...
        lc1, lc2, lc3 = st.columns(3); lc1.metric("信貸總額", f"${84 * 15000:,.0f}"); lc2.metric("已繳", f"${new_paid * 15000:,.0f}"); lc3.metric("剩餘", f"${(84 - new_paid) * 15000:,.0f}")
        
        st.write("---")
        st.markdown("##### 🤝 應收帳款戰情：彰化銀行信貸 (別人欠我的)")
        loan_chb_info = st.session_state.my_data['loan_chb']
        chb_amount = st.number_input("💸 對方每月應還金額", value=int(loan_chb_info.get('regular_amount', 0)), step=1000)
        ...
        new_paid_chb = st.slider("手動微調對方已還期數", min_value=0, max_value=60, value=int(loan_chb_info['months_paid']))
        ...
        cc1, cc2, cc3 = st.columns(3); cc1.metric("彰銀信貸總額", f"${60 * chb_amount:,.0f}"); cc2.metric("對方已還", f"${new_paid_chb * chb_amount:,.0f}"); cc3.metric("剩餘應收", f"${(60 - new_paid_chb) * chb_amount:,.0f}")
        ...
        with col_ex2: rec_item = st.text_input("項目名稱")
        with col_ex3: rec_amount = st.number_input("金額 (元)", min_value=0, step=10, value=0)
        with col_ex4:
            st.markdown("<div style='margin-top: 28px;'></div>", unsafe_allow_html=True)
            if st.button("➕ 記一筆", use_container_width=True) and rec_amount > 0 and rec_item:
                new_record = {"date": rec_date.strftime("%Y-%m-%d"), "item": rec_item, "amount": rec_amount}
                pf_data['expenses' if rec_type == "支出" else 'incomes'].append(new_record); save_to_json(st.session_state.my_data); st.rerun()

        curr_str = datetime.today().strftime("%Y-%m")
        t_exp = sum(e['amount'] for e in pf_data.get('expenses', []) if e['date'].startswith(curr_str))
        t_inc = sum(i['amount'] for i in pf_data.get('incomes', []) if i['date'].startswith(curr_str))
        rem = t_inc - t_exp
        
        bc1, bc2, bc3 = st.columns(3); bc1.metric("本月收入", f"${t_inc:,.0f}"); bc2.metric("本月支出", f"${t_exp:,.0f}"); bc3.metric("本月餘額", f"${rem:,.0f}")
        
        true_net_worth = g_mkt - ((84 - new_paid) * 15000) + ((60 - new_paid_chb) * chb_amount)
...
with st.expander("💰 買賣損益試算器", expanded=False):
    if not df.empty:
        ...
        current_price = target_row['現價']; current_cost = target_row['均價']; current_holdings = target_row['張數']
        ...
        if st.session_state.calc_trade_type == "賣出 (計算已實現損益)":
            trade_shares_display = min(st.session_state.calc_trade_shares, current_holdings)
            realized_profit = (current_price - current_cost) * trade_shares_display * 1000
            st.markdown(f"<div class='calc-box'>📝 試算賣出 **{trade_shares_display}** 張<br>🎉 預估損益：<div style='font-size: 24px; font-weight: bold; color: {'#d32f2f' if realized_profit>0 else '#388e3c'};'>{'+' if realized_profit>0 else ''}${realized_profit:,.0f}</div></div>", unsafe_allow_html=True)
            ...
            new_avg_cost = ((current_cost * current_holdings * 1000) + buy_cost_total) / ((current_holdings + trade_shares_display) * 1000)
            st.markdown(f"<div class='calc-box'>📝 試算買進 **{trade_shares_display}** 張<br>💸 預估買入總花費：${buy_cost_total:,.0f}<br>🎯 買入後全新均價：${new_avg_cost:.2f}</div>", unsafe_allow_html=True)
            st.button("💾 確認買進並更新庫存", type="primary", use_container_width=True, on_click=execute_trade)
    else: st.warning("請先新增庫存才能進行試算喔！")
st.write("---")

bot_c1, bot_c2, bot_c3 = st.columns([2, 2, 6])
with bot_c1:
    if st.button("🔄 手動更新【股價】", use_container_width=True): fetch_data.clear(); st.rerun()
with bot_c2:
    if st.button("💰 強制更新【除權息】", use_container_width=True, type="primary"):
        fetch_taiwan_upcoming_dividends.clear(); get_div_data.clear(); fetch_watchlist_dividend.clear(); fetch_data.clear()
        st.session_state.update_success = "除權息資料已更新！"; st.rerun()
with bot_c3:
    with st.expander("⚙️ 標的管理 (庫存新增 / 修改 / 刪除)", expanded=True):
        col_add1, col_add2, col_add3, col_add4, col_add5 = st.columns([2,3,2,2,2])
        with col_add1: st.text_input("代碼", key="add_sym_bot", on_change=auto_fill_etf_name)
        with col_add2: st.text_input("自定義名稱", key="add_name_bot")
        with col_add3: st.number_input("張數", step=1.0, key="add_h_bot")
        with col_add4: st.number_input("均價", step=0.1, key="add_c_bot")
        with col_add5:
            st.markdown("<div style='margin-top: 28px;'></div>", unsafe_allow_html=True)
        ...
        if st.session_state.my_data['etfs']:
            ...
            for i, item in enumerate(st.session_state.my_data['etfs']):
                with st.expander(f"📍 {item['name']}"):
                    ...
                    with ce1: st.number_input("張數", value=float(item['holdings']), step=1.0, key=f"edit_h_{i}")
                    with ce2: st.number_input("均價", value=float(item['cost']), step=0.1, key=f"edit_c_{i}")
...
if current_etfs:
    with st.spinner("正在繪製高精度股價戰報..."):
        try:
            ...
            df_melted = df_chart.melt(id_vars=[df_chart.columns[0]], var_name='ETF', value_name='Price')
            chart = alt.Chart(df_melted).mark_line().encode(x=alt.X(f'{df_chart.columns[0]}:T', axis=alt.Axis(format='%d日', title=None, grid=False)), y=alt.Y('Price:Q', scale=alt.Scale(zero=False), axis=alt.Axis(title=None, labelFontSize=10)), color=alt.Color('ETF:N', legend=alt.Legend(title=None, orient="bottom")), tooltip=[alt.Tooltip(f'{df_chart.columns[0]}:T', format='%Y/%m/%d', title='日期'), alt.Tooltip('ETF:N', title='標的'), alt.Tooltip('Price:Q', format='.2f', title='收盤價')]).properties(height=450).interactive()
            st.altair_chart(chart, use_container_width=True)
        except Exception as e: st.error(f"圖表產生失敗：{e}")
else: st.info("目前無庫存標的。")
