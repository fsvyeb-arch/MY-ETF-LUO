...
def get_div_data(symbol, custom_div_info=None):
    is_announced = False; div_amount = 0.0; ex_date = "待官方公告"; pay_date = "待官方公告"
    fill_status = "-"; status_msg = "⏳ 依前次估算"
    ...
    try:
        ...
        if custom_div_info and custom_div_info.get('v', 0) > 0:
            ...
            if ex_date_obj.date() >= today.date(): status_msg = "✅ 已公告 (手動)"
            ...
            ex_date_obj = datetime.strptime(ex_date, '%Y-%m-%d')
            if ex_date_obj.date() >= today.date(): status_msg = "✅ 已公告 (台灣官方)"
            else: status_msg = "✅ 前次配息 (台灣官方)"
        else:
            divs = tk.dividends
            if not divs.empty:
...
def fetch_watchlist_data(wl_list):
    ...
    for item in wl_list:
        try:
            ...
            results.append({"代號": item['symbol'].replace('.TW', ''), "名稱": item['name'], "現價": round(curr_p, 2), "漲跌": round(diff, 2), "漲跌幅": f"{pct:+.2f}%", "狀態": status_light})
...
def fetch_watchlist_dividend(wl_list, custom_divs):
    ...
    for item in wl_list:
        ...
        try:
            ...
            results.append({"類別": "👀 自選", "ETF 名稱": item['name'], "基金規模": cap_str, "配息頻率": freq, "配息月份": "、".join(map(str, months)) + " 月" if months else "未設定", "狀態": status_msg, "除息日": ex_date, "發放日": pay_date, "每股金額": f"${div_amount:.3f}", "最新填息紀錄": fill_status})
        except Exception: continue
    return pd.DataFrame(results)
...
today_val_str = f"+${g_today_pnl:,.0f}" if g_today_pnl >= 0 else f"-${abs(g_today_pnl):,.0f}"
...
total_val_str = f"+${total_net_profit:,.0f}" if total_net_profit >= 0 else f"-${abs(total_net_profit):,.0f}"
...
b10_lbl, b10_typ = ("🔽 收起每日股價與損益", "primary") if st.session_state.show_daily_price else ("🗓️ 展開每日股價與損益", "secondary") 
...
if st.session_state.show_tech:
    if not df.empty:
        ...
        with tech_col:
            ...
            if "設定高標(停利)" in df_tech.columns and "設定低標(停損)" in df_tech.columns: df_tech_display = df_tech.drop(columns=["設定高標(停利)", "設定低標(停損)"])
            else: df_tech_display = df_tech
        ...
        with auto_tech_col:
            ...
            auto_update = st.radio("即時更新", ["❌ 關閉", "✅ 開啟"], key="auto_refresh_mode", horizontal=False, label_visibility="collapsed")
...
if st.session_state.show_constituents:
    if not df.empty:
        ...
        for idx, item in enumerate(st.session_state.my_data['etfs']):
            ...
            base = alt.Chart(df_comp).encode(theta=alt.Theta("weight:Q", stack=True), color=alt.Color("name:N", sort=alt.EncodingSortField(field="weight", op="sum", order="descending"), legend=alt.Legend(title=None, orient="right", labelFontSize=12)), tooltip=[alt.Tooltip("name:N", title="成分股"), alt.Tooltip("weight:Q", title="權重 (%)", format=".2f")])
...
if st.session_state.show_daily_price:
    st.markdown("#### 🗓️ 庫存與自選 ETF 近一個月每日收盤價與單日損益")
    port_map, wl_map, port_holdings = {}, {}, {}
    ...
    for item in st.session_state.my_data.get('watchlist', []):
        if item['symbol'] not in port_map: wl_map[item['symbol']] = f"👀 {item['name']}"
    ...
    if current_symbols:
        with st.spinner("📡 正在向資料庫調閱近一個月每日股價與計算損益..."):
            try:
                ...
                hist_data = hist_data.iloc[::-1]; diff_data = diff_data.iloc[::-1]
                hist_data = hist_data.T; diff_data = diff_data.T
                ...
                if valid_port_names:
                    ...
                    for etf_name in valid_port_names:
                        ...
                        for date_col in hist_data.columns:
                            price = hist_data.loc[etf_name, date_col]
                            diff = diff_data.loc[etf_name, date_col]
                            if pd.isna(price): display_port.loc[etf_name, date_col] = "-"
                            else:
                                if pd.isna(diff) or shares == 0: display_port.loc[etf_name, date_col] = f"{price:.2f}"
                                else:
                                    pnl = diff * shares
                                    sign = "+" if pnl > 0 else ""
                                    display_port.loc[etf_name, date_col] = f"{price:.2f} ({sign}{pnl:,.0f})"
                ...
                if valid_wl_names:
                    ...
                    for col in display_wl.columns: display_wl[col] = display_wl[col].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "-")
                    styled_wl = display_wl.style.apply(color_prices, axis=None)
                    st.dataframe(styled_wl, use_container_width=True)
                st.caption("💡 提示：顯示近 1 個月交易日數據。括號內為依照您的庫存張數計算之「該日漲跌真實損益」。數值呈現紅色代表上漲/賺錢，綠色代表下跌/虧損。")
...
if st.session_state.show_pledge:
    if not df.empty:
        ...
        if borrowed != pledge_data.get('borrowed_amount', 0):
            ...
            st.rerun()
        pledge_df_list = []
        total_pledge_mkt = 0; total_borrowable = 0
        for item in st.session_state.my_data['etfs']:
            sym = item['symbol']; name = item['name']; h_total = item['holdings']
            p_shares = item.get('pledged_shares', 0.0); is_pledged = item.get('is_pledged', False)
            ...
            pledge_df_list.append({"✓ 選取": is_pledged, "ETF 名稱": name, "總庫存 (張)": h_total, "庫存市值 (元)": round(original_mkt, 0), "質押張數": p_shares, "現價": round(curr_p, 2), "質押市值 (元)": round(p_mkt, 0), "可借上限 (60%)": round(p_limit, 0) })
        ...
        if borrowed > 0:
            if margin_ratio < 130: col_m4.metric("🚨 目前維持率", f"{margin_ratio:.2f}%", "危險：低於 130% 將面臨斷頭", delta_color="inverse")
        ...
        edited_pledge = st.data_editor(pledge_df, column_config={"✓ 選取": st.column_config.CheckboxColumn("✓ 選取質押", help="勾選後才會計入擔保品總市值"), "質押張數": st.column_config.NumberColumn("質押張數 (雙擊編輯)", min_value=0.0, step=1.0, format="%.1f"), "庫存市值 (元)": st.column_config.NumberColumn("庫存市值 (元)", format="%.0f"), "現價": st.column_config.NumberColumn("現價", format="%.2f"), "質押市值 (元)": st.column_config.NumberColumn("質押市值 (元)", format="%.0f"), "可借上限 (60%)": st.column_config.NumberColumn("可借上限 (60%)", format="%.0f") }, disabled=["ETF 名稱", "總庫存 (張)", "庫存市值 (元)", "現價", "質押市值 (元)", "可借上限 (60%)"], use_container_width=True, hide_index=True)
        has_p_changes = False
...
if st.session_state.show_secret:
    ...
    if not st.session_state.is_unlocked:
        ...
        if new_paid != loan_info['months_paid']:
            st.session_state.my_data['loan']['months_paid'] = new_paid; save_to_json(st.session_state.my_data); st.rerun()
        ...
        if chb_amount != loan_chb_info.get('regular_amount', 0):
            st.session_state.my_data['loan_chb']['regular_amount'] = chb_amount; save_to_json(st.session_state.my_data); st.rerun()
        ...
        if new_paid_chb != loan_chb_info['months_paid']:
            st.session_state.my_data['loan_chb']['months_paid'] = new_paid_chb; save_to_json(st.session_state.my_data); st.rerun()
        ...
        with col_ex2:
            if rec_type == "支出": rec_item = st.text_input("支出項目", placeholder="例如: 買早晨咖啡、湛藍燒肉")
            else: rec_item = st.text_input("收入項目", placeholder="例如: 本薪、業務獎金、其他收入")
        ...
        with col_ex4:
            ...
            if st.button("➕ 記一筆", use_container_width=True):
                ...
                else: st.warning("請填寫項目與大於 0 的金額！")
        ...
        if remaining_budget >= 0: bc3.metric("本月剩餘可用餘額", f"${remaining_budget:,.0f}", "安全範圍", delta_color="normal")
        else: bc3.metric("本月剩餘可用餘額", f"${remaining_budget:,.0f}", "⚠️ 已超支", delta_color="inverse")
        ...
        with tab_in:
            if curr_month_incomes:
                ...
                edited_incomes = st.data_editor(df_incomes, num_rows="dynamic", use_container_width=True, hide_index=True, key="income_editor", column_config={"🗑️ 刪除": st.column_config.CheckboxColumn("打勾刪除此筆", default=False)})
                if st.button("💾 確認更新 / 刪除勾選的收入", use_container_width=True):
                    ...
                    for _, row in edited_incomes.iterrows():
                        if row.get("🗑️ 刪除", False): continue 
                        if pd.notna(row['日期']) and pd.notna(row['項目']) and pd.notna(row['金額']): updated_curr.append({"date": str(row['日期']), "item": str(row['項目']), "amount": int(row['金額'])})
                    pf_data['incomes'] = other_month_incomes + updated_curr; save_to_json(st.session_state.my_data); st.success("✅ 收入清單已成功更新！"); st.rerun()
            else: st.info("本月尚無任何收入紀錄。")
        with tab_ex:
            if curr_month_expenses:
                ...
                edited_expenses = st.data_editor(df_expenses, num_rows="dynamic", use_container_width=True, hide_index=True, key="expense_editor", column_config={"🗑️ 刪除": st.column_config.CheckboxColumn("打勾刪除此筆", default=False)})
                if st.button("💾 確認更新 / 刪除勾選的支出", use_container_width=True):
                    ...
                    for _, row in edited_expenses.iterrows():
                        if row.get("🗑️ 刪除", False): continue 
                        if pd.notna(row['日期']) and pd.notna(row['項目']) and pd.notna(row['金額']): updated_curr.append({"date": str(row['日期']), "item": str(row['項目']), "amount": int(row['金額'])})
                    pf_data['expenses'] = other_month_expenses + updated_curr; save_to_json(st.session_state.my_data); st.success("✅ 支出清單已成功更新！"); st.rerun()
            else: st.info("本月尚無任何支出紀錄。")
        ...
        if st.button("🔐 重新上鎖並關閉", use_container_width=True):
            st.session_state.is_unlocked = False; st.session_state.show_secret = False; st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    st.write("---")
...
with st.expander("💰 買賣損益試算器", expanded=False):
    ...
    if not df.empty:
        ...
        if st.session_state.calc_trade_type == "賣出 (計算已實現損益)":
            ...
            if realized_profit > 0: st.markdown(f"🎉 預估已實現損益 (賺)：<div class='calc-result-profit'>+${realized_profit:,.0f}</div>", unsafe_allow_html=True)
            elif realized_profit < 0: st.markdown(f"📉 預估已實現損益 (賠)：<div class='calc-result-loss'>-${abs(realized_profit):,.0f}</div>", unsafe_allow_html=True)
            else: st.markdown(f"⚖️ 預估已實現損益：<div style='font-size: 24px; font-weight: bold; margin-top: 10px;'>$0</div>", unsafe_allow_html=True)
...
with bot_c2:
    if st.button("💰 強制更新【除權息】", use_container_width=True, type="primary"):
        ...
        st.session_state.update_success = "除權息資料已更新至最新！"; st.rerun()
with bot_c3:
