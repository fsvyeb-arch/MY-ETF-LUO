...
if 'pledge' not in st.session_state.my_data:
    st.session_state.my_data['pledge'] = {
        "borrowed_amount": 0,
        "months_paid": 1,
        "total_months": 18,
    ...
    if "months_paid" not in st.session_state.my_data['pledge']:
        st.session_state.my_data['pledge'].update({
            "months_paid": 1,
            "total_months": 18,
            "last_updated_month": now_str
        })

pledge_data_init = st.session_state.my_data['pledge']
last_m_pledge = pledge_data_init.get('last_updated_month', now_str)
...
if last_m_pledge != now_str:
    ...
    diff_months = (y_curr - y_last) * 12 + (m_curr - m_last)
    if diff_months > 0:
        pledge_data_init['months_paid'] = min(pledge_data_init.get('total_months', 18), pledge_data_init.get('months_paid', 1) + diff_months)
        pledge_data_init['last_updated_month'] = now_str
        save_to_json(st.session_state.my_data)
...
def fetch_data(etf_list, custom_divs):
    ...
    return pd.DataFrame(results), pd.DataFrame(tech_results), 0, 0, 0, 0, [], [], [], {}df, df_tech, g_mkt, g_cost, g_div, g_today_pnl, radar_ex, radar_pay, price_alerts, monthly_calendar = fetch_data(st.session_state.my_data['etfs'], st.session_state.my_data.get('custom_divs', {}))
